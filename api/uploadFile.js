...
function getLightTran(tranObj){
    let lightProduct = peptideMassFunctions.getTransLightMz(tranObj.Peptide, tranObj.Modification, 4,tranObj.Product_Ion,10);
    let keys1 = Object.keys(lightProduct);
    let keys2 = (keys1.length > 0)?Object.keys(lightProduct[keys1[0]]):[];
    let productLight = (keys1.length == 1 && keys2.length == 1)?lightProduct[keys1[0]][keys2[0]].lightMW:'error';
    let lightTran = {
        CatalogNumber: tranObj.CatalogNumber,
        Peptide: tranObj.Peptide,
        istd: 'FALSE',
        Precursor_Ion: peptideMassFunctions.getPrecursorLightMz(tranObj.Peptide,tranObj.Modification,tranObj.Precursor_Ion),
        MS1_Res: tranObj.MS1_Res,
        Product_Ion: productLight,
        MS2_Res: tranObj.MS2_Res,
        Dwell: tranObj.Dwell,
        OptimizedCE: tranObj.OptimizedCE,
        Cell_Accelerator_Voltage: tranObj.Cell_Accelerator_Voltage,
        Ion_Name: tranObj.Ion_Name,
        FromFile: 'Calculated By Server',
        ModificationString: tranObj.ModificationString,
        Modification: tranObj.Modification,
        PeptideType: 'light',
        peptideId: tranObj.peptideId
    };
    return lightTran;  //Product_Ion will be 'error' if no light tran or more than one light tran
}


function addToTransObj(transObj, curTrans){
    //transObj = {CatalogNumber:[{light:{CatalogNumber:,Peptide:,istd:...},heavy:{CatalogNumber:,Peptide:,istd:...}}]}
    //curTrans = {light:{CatalogNumber:,Peptide:,istd:...},heavy (optional):} <- if a light transition is read
    //            no heavy transition will be recorded.

    //if for a CatalogNumber, the light trans from curTrans matches to a light trans in transObj,
    //replace the light trans in transObj unless the light trans in transObj is not generated by server
}
    ...
    upload(req, res, async function (err){
        ...
        if(err){
            ...
            if(isValidateFile(headerArr,lines[0],headerKey)){
                ...
                let linesArr = []
                let transObj = {}; //{CatalogNumber:[{light:{CatalogNumber:,Peptide:,istd:...},heavy:{CatalogNumber:,Peptide:,istd:...}}]}
                ...
                for(let i = 0; i < lines.length-1; i++){
                    ...
                    let modInfo = await getModificationId(thisTran.Peptide,thisTran.ModificationString);
                    thisTran.Modification = modInfo.modification;
                    thisTran.PeptideType = modInfo.PeptideType;
                    thisTran.peptideId = await addPeptideInfo(thisTran);

                    let curTrans = {};
                    if(thisTran.istd.toUpperCase() == 'FALSE'){
                        curTrans.light = thisTran;
                    }else{
                        //get the light object
                        curTrans.light = getLightTran(thisTran);
                        curTrans.heavy = thisTran;
                    }

                    addToTransObj(transObj, curTrans); //this checks to see if curTrans in transObj and add/change to transObj

                }

                //push each trans to linesArr
                for(let peptide in transObj){
                    for(let i = 0; i < transObj[peptide].length; i++){
                        for(let tran in transObj[peptide][i]){
                            linesArr.push(transObj[peptide][i][tran]);
                        }
                    }
                }*/
